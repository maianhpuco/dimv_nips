...
def classify(algo,
    ...
    exp_dir = get_save_path(ds_name, 0, exp_num, "exp")
    ...
    print("train, test indice {} {} ".format(len(train_indices),
                                             len(test_indices)))
    ...
    for mrate in missing_rates:
        ...
        with open(os.path.join(save_folder, "acc_{}.json".format(algo)),
                  'w') as f:
            json.dump({"acc": acc}, f)
        print(">> -------Complete save --- {} ----with acc {}  with at path {}".
